...
public class LoginController extends HttpServlet {
	...
	/*
	 * @RequestMapping(value = "login") public String ecourt() {
	 * 
	 * return "/ecourt/login"; }
	 */
	...
	public String validateNotice(HttpServletRequest req) {
		...
		String smstext = " OTP for viewing the complete file is " + otp
				+ " which is valid for 5 minutes. Please do not share the OTP with anyone.-AHC";
	...
	public @ResponseBody String validateAdvocate(@RequestBody Register register) {
		...
		try {
			...
			/*
			 * date3 = sdf.parse("2020-07-14 00:00:45"); date2 =
			 * sdf.parse("2020-07-15 00:00:01");
			 */
	...
	public String noticelogin(@RequestBody Notice nt, HttpSession session) throws ParseException {
		...
		/*
		 * Date date3 = sdf.parse("2020-07-14 00:00:45"); Date date2 =
		 * sdf.parse("2020-07-15 00:00:01");
		 */
		if (current.after(date3) && current.before(date2)) {
			response.setResponse("FALSE");
			...
			if (nt != null) {
				...
				String destPath = context.getRealPath("") + File.separator + "uploads" + File.separator + nt.getNt_id()
						+ ".pdf";
...
//				 logger.error("This is an error log entry"+user.getUm_fullname());
	...
	@RequestMapping(value = "/dms/userlogin", method = RequestMethod.POST, consumes = {
			"application/json;charset=UTF-8" }, produces = { "application/json;charset=UTF-8" })
	public @ResponseBody String userlogin(@RequestBody User user, HttpSession session, HttpServletRequest request)
		...
		if (current.after(date3) && current.before(date2)) {
			 ...
			 * response.setResponse("FALSE"); response.
			 * setData("E-Filing Can Be Done Between 10 Am To 4 PM, As Per Guidelines of Allahabad High Court"
			 * ); }
			 */
		else {
...
//				 logger.error("This is an error log entry"+user.getUm_fullname());
	...
	@RequestMapping(value = "/dms/userFagPass", method = RequestMethod.POST, consumes = {
			"application/json;charset=UTF-8" }, produces = { "application/json;charset=UTF-8" })
	 ...
	 * 
	 * @ResponseBody public String validateOtp(@RequestBody User u,HttpSession
	 * session) { String result = " "; User user =
	 * userService.validateUser(u.getUsername()); AdvocateEfiling
	 * adv=userService.getAdvocateByRollNo(u.getUsername()); ActionResponse <User>
	 * response = new ActionResponse(); if(user.getUm_id() != null) {
	 * if(u.getUm_otp().equals(user.getUm_otp())) { user.setUm_rec_status(1);
	 * user=userService.save(user); response.setData(user);
	 * response.setResponse("TRUE");
	 * 
	 * String pass=cm.generatePassword(); Lookup
	 * urlLookup=lookupService.getLookUpObject("SMS_URL"); String
	 * sms_url=urlLookup.getLk_longname(); String mob_no=user.getUm_mobile(); String
	 * smstext="Dear "+user.getUm_fullname()+" your new Password for eFiling is "
	 * +pass+" Please do not share this password with anybody.";
	 * 
	 * String pwd = cm.md5encryption(pass); user.setPassword(pwd);
	 * user.setUm_otp(null);
	 * 
	 * user=userService.save(user); cm.sendSMS(sms_url, mob_no, smstext);
	 * response.setResponse("TRUE");
	 ...
	 * } else if(adv!=null) {
	 * 
	 * if(u.getUm_otp().equals(adv.getOtp())) { response.setData(adv);
	 * response.setResponse("TRUE");
	 * 
	 * String pass=cm.generatePassword(); Lookup
	 * urlLookup=lookupService.getLookUpObject("SMS_URL"); String
	 ...
	 * 
	 * 
	 * } else{ response.setResponse("FALSE"); response.setData("Error"); } result =
	 * cm.convert_to_json(response); //System.out.println(result); return result;
	 * 
	 * }
	 */
