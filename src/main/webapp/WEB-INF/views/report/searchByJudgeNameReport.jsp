<%@ include file="../../content/header.jsp"%>

<!-- <script src="daterangepicker.js"></script> -->

<div id="content" class="content"
	ng-controller="searchByJudgeNameReportCtrl">
	<!-- begin row -->
	<div class="row">
		<div class="col-md-11">
			<!-- begin panel -->
			<div class="panel panel-inverse">
				<div class="panel-heading">
					<h4 class="panel-title">Search By JudgeName Report</h4>
				</div>
				<div class="panel-body">
					<div>
						<table id="data-table" st-table="displayedCollection"
							st-safe-src="masterdata"
							class="table table-striped table-bordered nowrap table-hover"
							class="p-20" width="100%">
							<thead>
								<tr>

									<th class="m-l-25">Judge Name</th>
								<!-- 	<th style="width: 16%"><select
										ng-options="bcd.jg_id as bcd.jg_name for bcd in judgeDataList"
										class="form-control" id="judgeId" name="judgeId"
										ng-model="model.judgeId" required>
									</select></th> -->
									<th>
										
											<div class="col-md-5 col-sm-5" class="multidropdown">
												<div ng-dropdown-multiselect="" options="obj"
													selected-model="validationmodel"  ng-model="dropdownlist"></div>
												<!-- 
			<dropdown-multiselect dropdown-title="Select Something"
				 model="selected_items" options="validationList"></dropdown-multiselect>
			<pre>selected roles = {{selected_items | json}}</pre>
 -->
											</div>
										



									</th>

									<th st-sort="lk_longname" class="m-l-25">Bench Code</th>
									<th style="width: 16%"><select
										ng-options="bcd.lk_id as bcd.lk_longname for bcd in branchDataList"
										class="form-control" id="benchCodeId" name="benchCodeId"
										ng-model="model.benchCodeId" required>
									</select></th>

									
								</tr>
								
								<tr>
								<th style="border: 0px">Date From</th>
									<th style="border: 0px">
										<div class="row" style="width: 125px">
											<div class="input-group date">
												<input type="text" class="form-control"
													datepicker-popup="{{format}}" ng-model="model.fromDate"
													is-open="opened1" datepicker-options="dateOptions"
													ng-disabled="true" /> <span class="input-group-addon"
													ng-click="open($event,'opened1')"><i
													class="glyphicon glyphicon-calendar"></i></span>
											</div>
										</div>
									</th>
									<th style="border: 0px">To</th>
									<th><div class="row" style="width: 125px">
											<div class="input-group date">
												<input type="text" class="form-control"
													datepicker-popup="{{format}}" ng-model="model.toDate"
													is-open="opened2" datepicker-options="dateOptions"
													ng-disabled="true" /> <span class="input-group-addon"
													ng-click="open($event,'opened2')"> <i
													class="glyphicon glyphicon-calendar"></i></span>
											</div></th>

									<th style="border: 0px">
										<button type="submit" class="btn btn-sm btn-success"
											ng-click="getSearchByJudgeNameReportData()" ng-disabled="!model.benchCodeId">
											<span class="glyphicon glyphicon-plus-sign"></span>Search
											here
										</button>

									</th>
									

									<th style="border: 0px"><button style="float: left"
											type="button" class="btn btn-primary btn-sm pull-right"
											csv-header="['Case No','Case Year','First Petitioner','First Respondent','Case Type','Judgement Date']"
											name="ExportToCsv1" ng-csv="judgereportListData" filename="test.csv"
											data-toggle="modal">
											<i class="fa fa-edit"></i> Excel
										</button></th> 								
								
								</tr>
							</thead>
						</table>

					</div>

					<table id="data-table" st-table="" st-safe-src="masterdata"
						class="table table-striped table-bordered nowrap table-hover"
						width="100%">
						<tr>
							<th>Case No</th>
							<th>Case Year</th>
							<th>First Petitioner</th>
							<th>First Respondent</th>
							<th>Case Type</th>
							<th>Judgement Date</th>
							<th>Action</th>
						</tr>
						<tbody>
							<tr ng-repeat="data in displayedCollection " class="odd gradeX">

								<!-- <td style="width: 30%">{{data.parameter1}}</td> -->
								<td>{{data.caseNo}}</td>
								 <td>{{data.caseYear}}</td>
								<td>{{data.firstPetitioner}}</td>
								<td>{{data.firstRespondent}}</td>
								<td>{{data.caseType}}</td>
								<td>{{data.judgementDate | date:'dd/MM/yyyy'}}</td>					
								<td>
									<a target="_new" href="${pageContext.request.contextPath}/casefile/dataentry/{{data.fileId}}" class="btn btn-info btn-xs" ><span class="glyphicon" aria-hidden="true"></span>View</a>
								</td>							</tr>
						</tbody>
					</table>
				</div>
			</div>
		</div>

	</div>
</div>

<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/scripts/controllers/searchByJudgeNameReport.js"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/Smart-Table-master/dist/smart-table.js"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/angularJs/ng-csv.js"></script>
	<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/angularJs/angular-sanitize.min.js"></script>

 <script src="https://cdnjs.cloudflare.com/ajax/libs/lodash.js/3.10.1/lodash.js"></script>

<script type="text/javascript"
src="${pageContext.request.contextPath}/js/angularJs/angularjs-dropdown-multiselect.min.js"></script>







